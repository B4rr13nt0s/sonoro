...
`globals.css` trae además unos pocos tokens del handoff que esta tabla no lista (nav, píldoras de marca, «Seguir comprando», botón con contorno), cada uno con su uso anotado. Ningún componente escribe un hex a mano. El único que se apartó del handoff es `texto-terciario-sobre-negro` (`#7A7A7E`, antes `#6E6E72`): el original daba 3.87:1 sobre negro en texto de 11 px.
...
- **El 404 de un listado lo da `proxy.ts`, no la página.** Los cuatro listados tienen `loading.tsx`, y en cuanto se dibuja el esqueleto las cabeceras ya salieron con 200: un `notFound()` en la página llega tarde y queda un soft 404. El proxy responde 404 a la categoría o marca inexistente, al filtro con un slug que no existe y a la página más allá de la última, con los conteos que el importador escribe en `data/conteos.json` (`lib/catalog/conteos.ts`). Un test compara esos conteos contra `listProducts` en cada combinación, así que cambiar cómo filtra el adaptador sin tocar los conteos rompe CI. Un listado vacío legítimo (marca sin productos en esa categoría) sí existe, con 200 y `noindex`.
